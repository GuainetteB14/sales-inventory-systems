...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('Cash Order Payment ') }}
                    <a href="{{ url('/cash') }}" class="float-right btn btn-sm btn-primary"><i
                            class="fas fa-arrow-left "> Back</i></a>
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ url('/payment-confirm') }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="row mb-3">
                            ...
                            @php
                            if ($order->discount > 0){
                            $total = $order->quantity * $new_price;
                            }else{
                        ...
                        <div class="row mb-3">
                            <label for="change" class="col-md-4 col-form-label text-md-right">{{ __('Change | ₱')
                                }}</label>
                            <div class="col-md-6">
                                <input id="change" type="text"
                                    class="form-control @error('change') is-invalid @enderror" autofocus name="change">
                            </div>
                        </div>
...
<script>
    $(document).ready( function () {
        ...
        $('input[name=cash_pay]').change(function() { 
           ...
           var total_change =   cash_pay - total
           $('#change').val(total_change)
         });
</script>
@endpush